...
export class DeliveriesListComponent implements OnInit {
  ...
  loadData() {
    ...
    this.tripService.getTrips().subscribe((t: TripResponse[]) => this.trips = t);
  ...
  openDetailsModal(delivery: DeliveryResponse) {
    ...
    this.deliveryPath = []; // Holds planned and actual paths
    this.tempMarkers = [];
    ...
    if (delivery.pickupLatitude && delivery.deliveryLatitude) {
        ...
        this.geocodingService.search(delivery.pickupAddress).subscribe(pRes => {
            if (pRes?.[0]) {
                const pLat = parseFloat(pRes[0].lat);
                const pLng = parseFloat(pRes[0].lon);
                this.geocodingService.search(delivery.deliveryAddress).subscribe(dRes => {
                    if (dRes?.[0]) {
                        const dLat = parseFloat(dRes[0].lat);
                        const dLng = parseFloat(dRes[0].lon);
    ...
    request$.subscribe({
      ...
      error: (err) => {
        ...
        Swal.fire({
          icon: 'error',
          title: 'Erreur',
          text: err.error?.message || err.error?.error || 'Une erreur est survenue lors de l\'enregistrement.',
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.deliveryService.deleteDelivery(id).subscribe({
          next: () => {
            this.loadData();
            Swal.fire({ icon: 'success', title: 'Supprimée', text: 'La livraison a été effacée.', timer: 1500, showConfirmButton: false });
          },
          error: (err) => {
            ...
            Swal.fire({
              icon: 'error',
              title: 'Erreur',
              text: err.error?.message || 'Impossible de supprimer cette livraison.',
              confirmButtonColor: '#e11d48'
            });
          }
        });
